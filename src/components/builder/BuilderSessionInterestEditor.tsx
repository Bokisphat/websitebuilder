...
import type { SessionInterestSectionProps } from "@/components/sections/SessionInterestSection";
...
function read(raw: Record<string, unknown>) {
  return {
    ...
    anchorId: typeof raw.anchorId === "string" ? raw.anchorId : "session-interest",
    optionPhoneLabel: typeof raw.optionPhoneLabel === "string" ? raw.optionPhoneLabel : "",
    optionZoomLabel: typeof raw.optionZoomLabel === "string" ? raw.optionZoomLabel : "",
...
export function BuilderSessionInterestEditor({ site, pageId, sectionId, onChange, props: rawProps, onRequestPexels }: Props) {
  const p = read(rawProps);

  const patch = (partial: Partial<SessionInterestSectionProps>) => {
  ...
  return (
    <div className="mt-2 space-y-3 rounded-lg border border-[var(--fusion-builder-accent)]/25 bg-zinc-50/95 p-3">
      <p className="text-[11px] font-medium uppercase tracking-wide text-[var(--fusion-builder-accent)]">Session interest</p>
      ...
      <label className="block">
        <span className="mb-1 block text-[10px] uppercase text-zinc-500">Anchor ID</span>
        <input
          value={p.anchorId}
          onChange={(e) => patch({ anchorId: e.target.value.trim() || "session-interest" })}
          className="w-full rounded-lg border border-zinc-300 bg-white px-2 py-1.5 text-xs text-zinc-900 outline-none focus:border-[var(--fusion-builder-accent)]/70"
        />
      </label>
      <label className="block">
        <span className="mb-1 block text-[10px] uppercase text-zinc-500">Phone option label</span>
        <input
          value={p.optionPhoneLabel}
          onChange={(e) => patch({ optionPhoneLabel: e.target.value })}
          className="w-full rounded-lg border border-zinc-300 bg-white px-2 py-1.5 text-xs text-zinc-900 outline-none focus:border-[var(--fusion-builder-accent)]/70"
        />
      </label>
      <label className="block">
        <span className="mb-1 block text-[10px] uppercase text-zinc-500">Zoom option label</span>
        <input
          value={p.optionZoomLabel}
          onChange={(e) => patch({ optionZoomLabel: e.target.value })}
          className="w-full rounded-lg border border-zinc-300 bg-white px-2 py-1.5 text-xs text-zinc-900 outline-none focus:border-[var(--fusion-builder-accent)]/70"
        />
      </label>
      <label className="block">
        <span className="mb-1 block text-[10px] uppercase text-zinc-500">Submit button label</span>
        <input
          value={p.buttonLabel}
      ...
      <SectionImageFields
        ...
        onPatch={(partial) => patch(partial as Partial<SessionInterestSectionProps>)}
